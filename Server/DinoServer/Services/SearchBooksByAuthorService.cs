...
public class SearchBooksByAuthorService : ISearchBooksService
{
    private readonly IDbContextFactory<UserContext> _contextFactory;
    private readonly IBookSearch _bookSearch;

    public SearchBooksByAuthorService(IDbContextFactory<UserContext> contextFactory, IBookSearch bookSearch)
    {
    ...
    public async Task<IEnumerable<User>> SearchBooksAsync(int userId, string searchQuery)
    {
        ...
        db.UserId = userId;
        return await _bookSearch.SearchAsync(db, searchQuery, (User book) => book.Author);
    }
}